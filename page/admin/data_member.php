...
<div class="panel panel-primary">
    ...
    <div class="panel-body">
        <table class="table table-bordered table-hover table-striped">
            <tr>
                <td colspan="5">
                    <form action="index.php?admin=cari_member_by_nama" method="post" class="form-inline">
            ...
            $mdaof = new MemberDaoImpl();
            $jml = $mdao->jumlahMember();
            ...
            if (!$page)
                $page = 1;
            $start = ($page - 1) * $batch;
            $data = $mdaof->findAllLimit($start, $batch);
            for ($i = 0; $i < count($data); $i++) {
                ...
                <tr>
                    ...
                    <td><a href="index.php?admin=member_detail&kode_member=<?php echo $kodeMember; ?>" class="btn btn-success" target="blank">Detail</a> | 
                        <a href="cetak_member.php?kode_member=<?php echo $kodeMember; ?>" class="btn btn-success" target="blank">Cetak</a>
                    </td>
                </tr>
                <?php
            }
            if ($pages > 1) {
                ...
                for ($i = 1; $i <= $pages; $i++) {
                    if ($i != ceil($start / $batch)) {
                        echo "<a href='index.php?admin=data_member&page=$i' class='btn btn-success'>$i</a> ";
                    } else {
                        echo " <a href='index.php?admin=data_member&page=$i' class='btn btn-success'>$i</a> ";
                    }
                }
